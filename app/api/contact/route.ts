...
export async function POST(request: NextRequest) {
  try {
    ...
    const { firstName, lastName, email, phone, service, budget, message } = extractFormData(formData);
    ...
    await transporter.sendMail({
      ...
      // html: createEmailHtml(...) // If you want to send HTML email
    });
    
   //add the form data to json file
    await appendFormDataToJson(firstName, lastName, email, phone, service, budget, message);

    return NextResponse.json({ message: 'Message sent successfully!' }, { status: 200 });
  } catch (error: any) {
    console.error('Error handling contact form:', error.message); // Log the error message
